proc create_ipi_design { offsetfile design_name } {
	...
	set axi_encoder_0 [ create_bd_cell -type ip -vlnv user.org:user:axi_encoder:1.0 axi_encoder_0]
	...
	connect_bd_intf_net [get_bd_intf_pins master_0/M_AXI ] [get_bd_intf_pins axi_encoder_0/S_AXI]

	# Create port connections
	connect_bd_net -net aclk_net [get_bd_ports ACLK] [get_bd_pins master_0/ACLK] [get_bd_pins axi_encoder_0/S_AXI_ACLK]
	connect_bd_net -net aresetn_net [get_bd_ports ARESETN] [get_bd_pins master_0/ARESETN] [get_bd_pins axi_encoder_0/S_AXI_ARESETN]
...
set_property -name {xsim.simulate.runtime} -value {100ms} -objects [get_filesets sim_1]
	...
	set offset_file "${bd_path}/axi_encoder_v1_0_tb_include.svh"
	set fp [open $offset_file "w"]
	puts $fp "`ifndef axi_encoder_v1_0_tb_include_vh_"
	puts $fp "`define axi_encoder_v1_0_tb_include_vh_\n"
...
set ip_path [file dirname [file normalize [get_property XML_FILE_NAME [ipx::get_cores user.org:user:axi_encoder:1.0]]]]
set test_bench_file ${ip_path}/example_designs/bfm_design/axi_encoder_v1_0_tb.sv
...
for { set i 1 } { 1 } { incr i } {
	set design_name "axi_encoder_v1_0_bfm_${i}"
...
remove_files -quiet -fileset sim_1 axi_encoder_v1_0_tb_include.vh
import_files -fileset sim_1 -norecurse -force $interface_address_vh_file
set_property top axi_encoder_v1_0_tb [get_filesets sim_1]
